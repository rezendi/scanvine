...
def get_twitter_thread(tweet_id, sharer_id, root_tweet_id, root_tweet_text, root_tweet_url):
    ...
    try:
        ...
        since = since - datetime.timedelta(hours=12)
...
def set_scores(when=datetime.datetime.utcnow(), past=14, future=30):
    ...
    try:
        ...
        articles = Article.objects.filter(
            ...
            Q(status=Article.Status.AUTHOR_ASSOCIATED) | Q (status=Article.Status.PUBLISHER_ASSOCIATED)
        ).defer('url','initial_url','title','contents','metadata','thumbnail_url','language','created_at','updated_at')
        log_job(job, "articles %s" % len(articles))
        
        for article in articles:
            articles_dict[article.id] = {}
            for key in ['total'] + CATEGORIES:
                articles_dict[article.id][key] = 0
                articles_dict[article.id]['%s_shares' % key] = 0
        
        shares = Share.objects.filter(article_id__in=articles_dict.keys()).values('id','sharer_id','article_id')
